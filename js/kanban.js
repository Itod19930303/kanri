...
function renderKanban(tickets) {
  ...
  KANBAN_COLUMNS.forEach(col => {
    ...
    colEl.innerHTML = `
      ...
      <div class="kanban-col flex flex-col gap-2 min-h-[120px] rounded-xl bg-base-200 p-2" data-status="${col.id}">
        ${colTickets.map(t => ticketCard(t)).join('')}
      </div>
    `;
    ...
    <div class="card bg-base-100 shadow-sm border-l-4 ${priorityClass} cursor-grab active:cursor-grabbing" data-id="${t.id}">
      <div class="card-body p-3 gap-1">
        <div class="flex justify-between items-start gap-1">
          <p class="font-semibold text-sm leading-snug flex-1">${escHtml(t.title)}</p>
